...
export class SolanaClient extends BlockchainClientBase<Blockchain.SOLANA> {
  ...
  /**
   * A namespaced collection of functions related to Solana's Staking program.
   */
  public Stake = new Stake(this.buildCtx.bind(this));
}
...
class Stake {
  ...
  async createAndDelegate(req: {
    ...
    return await Solana.Stake.createAndDelegate(
      await this.buildCtx(req.authority),
      {
  ...
  async deactivate(req: { authority: string; stakeAccount: string }) {
    return await Solana.Stake.deactivate(await this.buildCtx(req.authority), {
  ...
  async merge(req: {
    ...
    return await Solana.Stake.merge(await this.buildCtx(req.authority), {
  ...
  async withdraw(req: { authority: string; stakeAccount: string }) {
    return await Solana.Stake.withdraw(await this.buildCtx(req.authority), {
      stakeAccount: new PublicKey(req.stakeAccount),
    });
  }
}
...
class BackpackSolanaWallet {
  ...
  public async signTransaction<T extends Transaction | VersionedTransaction>(
    ...
    const signature = await this.secureSvmClient.signTransaction(
      {
        ...
        tx: txStr,
        uuid: request.uuid,
        disableFeeInjection: request.disableFeeInjection,
      },
      { uiOptions }
    );
    ...
    const finalTx =
      request.tx instanceof VersionedTransaction
        ? deserializeTransaction(signature.response.transactionEncoding)
        : deserializeLegacyTransaction(signature.response.transactionEncoding);
  ...
  >(
    ...
    const txs = signatures.response.signatures.map(
      ({ signature, transactionEncoding }, i) => {
        const tx =
          request.txs[i] instanceof VersionedTransaction
            ? deserializeTransaction(transactionEncoding)
            : deserializeLegacyTransaction(transactionEncoding);
